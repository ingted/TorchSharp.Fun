...
module DGX =
    ...
    type ManagedHint =
        {
            ReadMostly: bool
            PrefetchDevice: int option
        }

    let defaultManagedHint =
        {
            ReadMostly = true
            PrefetchDevice = Some 0
        }

    let resolveDType (policy: DTypePolicy) (input: torch.Tensor) =
        match policy with
        | Keep -> input.dtype
        | Float16 -> torch.float16
        | Float32 -> torch.float32

    /// Best-effort zero-copy promotion for DGX/UM path.
    let promoteManaged (hint: ManagedHint) (input: torch.Tensor) =
        UnifiedMemory.tryPromoteToManaged input hint.ReadMostly hint.PrefetchDevice

    /// Stage: optional UM promotion as composable IModel.
    let managedInput (hint: ManagedHint option) =
        let h = defaultArg hint defaultManagedHint
        F [] [] (fun x -> promoteManaged h x)

    let managedInputDefault () = managedInput None

    /// Stage: convert input dtype policy before a compute-heavy section.
    let castInput (policy: DTypePolicy) =
        F [] [] (fun x ->
            let target = resolveDType policy x
            if x.dtype = target then x else x.to_type(target))

    /// Stage: NVFP4 quantize->dequantize adapter for "NVFP4 input, FP16/FP32 compute".
    let nvfp4Input (computePolicy: DTypePolicy) =
        F [] [] (fun x ->
            let computeDType = resolveDType computePolicy x
            use xCompute = if x.dtype = computeDType then x else x.to_type(computeDType)
            let q, s = Nvfp4Training.quantizePacked xCompute
            use qd = q
            use sd = s
            Nvfp4Training.dequantizePacked qd sd computeDType)

    /// Utility: pack any float output to NVFP4 sink tensors.
    let packNvfp4Sink (policy: DTypePolicy) (output: torch.Tensor) =
        let computeDType = resolveDType policy output
        use y = if output.dtype = computeDType then output else output.to_type(computeDType)
        Nvfp4Training.quantizePacked y

    /// Utility: optionally empty native NVFP4 cache for long-running sessions.
    let tryEmptyNvfp4Cache () = NativeInterop.tryEmptyNvfp4Cache()
